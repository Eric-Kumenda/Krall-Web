...
export async function middleware(request: NextRequest) {
	...
	if (request.nextUrl.pathname.startsWith("/api")) {
		// Allow public routes (auth, payment) without IP/Auth check
		const publicRoutes = [
			"/api/auth/send-code",
		...
		if (
			...
			// Check IP for Client App (Optional strict mode)
			const clientIp =
				request.headers.get("x-forwarded-for") || (request as any).ip;

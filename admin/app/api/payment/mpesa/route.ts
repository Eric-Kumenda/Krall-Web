...
export async function POST(request: Request) {
  try {
    const { phoneNumber, amount, eventId, email, attendees } = await request.json();
    ...
    if (!consumerKey || !consumerSecret || !passkey || !shortcode || !callbackUrl) {
       ...
       console.log('[MOCK MPESA] Simulating STK Push for:', phoneNumber, amount);
    ...
    if (stkData.ResponseCode !== '0') {
      return NextResponse.json({ error: 'STK Push failed', details: stkData }, { status: 400 });
    }

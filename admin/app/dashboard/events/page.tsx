...
import { Plus, Search, Edit, Trash2, Eye, EyeOff, Calendar as CalendarIcon, MapPin, Ticket } from 'lucide-react'
...
export default function EventsPage() {
  ...
  const togglePublish = async (id: string, currentState: boolean) => {
    dispatch(updateEvent({ id, data: { is_published: !currentState } }))
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold font-montserrat text-white">Events</h1>
          <p className="text-gray-400 mt-1">Manage your events and tickets</p>
        </div>
        <Link 
      ...
      {loading && events.length === 0 ? (
        ...
        <div className="text-center py-12 glass-card rounded-2xl">
          ...
          <Link href="/dashboard/events/new" className="text-primary hover:underline">Create your first event</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          {filteredEvents.map((event) => (
            <div key={event.id} className="glass-card p-4 rounded-xl flex flex-col md:flex-row gap-6 items-start md:items-center group hover:border-primary/30 transition-colors">
              {/* Image */}
              <div className="w-full md:w-32 h-32 md:h-24 bg-gray-800 rounded-lg overflow-hidden flex-shrink-0">
                {event.image_url ? (
                  <Image src={event.image_url} alt={event.title} width={128} height={96} className="object-cover w-full h-full" />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className={`px-2 py-0.5 text-xs rounded-full ${event.is_published ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
              ...
              <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                ...
                >
                  {event.is_published ? <Eye size={20} /> : <EyeOff size={20} />}
                </button>
                <Link 
